# %% [markdown]
# # Regression tests for the longitudinal ascertainment simulation contract
...
SPEC = importlib.util.spec_from_file_location("ascertainment_stress", ROOT / "code" / "ascertainment_stress.py")
...
def simulate(hours: list[float]) -> pd.DataFrame:
    ...
    return CORE.delete_and_reconstruct(
...
def test_observed_history_generator_targets_retention() -> None:
    patient = pd.concat([patient_frame().assign(reference_id=i) for i in range(1, 101)], ignore_index=True)
    serial = pd.DataFrame(
        [(i, hour, 1.0 + 0.01 * position) for i in range(1, 101) for position, hour in enumerate((12, 36, 60, 84, 120))],
        columns=["reference_id", "hour", "creatinine"],
    )
    result = CORE.delete_and_reconstruct(
...
if __name__ == "__main__":
    test_second_window_ends_at_96_hours()
    test_168_hour_measurement_can_define_outcome_but_not_observability()
    test_observed_history_generator_targets_retention()
    print("simulation contract tests: PASS")
