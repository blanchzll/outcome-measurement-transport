...
# applied unchanged to MIMIC-IV and the locked 3710-patient source analysis
...
ROOT = release_path('analysis')
STAGE = ROOT / 'outputs/source3710_clinical_bridge'
INSPIRE_ROOT = release_path('inspire')
MIMIC_ROOT = release_path('mimic')
SOURCE_PATH = release_path('source', 'clean_clinical_data.csv')
for _subdir in ('tables', 'outputs', 'secure_work'):
    (STAGE / _subdir).mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    audit = {
        ...
        'input_sha256': {
            'source_3710': sha256(SOURCE_PATH),
