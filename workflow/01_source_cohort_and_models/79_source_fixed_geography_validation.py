...
# %%
source = engineer_loco_features(
    load_cohort(BASE / "secure_source" / "inter3_deidentified_4014.csv")
...
results.to_csv(ROOT / "tables" / "Table_source_fixed_geography_validation.csv", index=False)
prediction_frame.to_csv(
    ROOT / "secure_work" / "SOURCE_4014_FIXED_GEOGRAPHY_PREDICTIONS_SECURE.csv.gz",
    index=False,
    compression="gzip",
)
...
audit = {
    ...
    "interval_estimand": (
        "Percentile intervals from analytic-record resampling within test centre, conditional on the fitted "
        "development model; they do not include repeated-patient clustering because no patient identifier exists."
    ),
    "patient_level_outputs_delivered": False,
    "fits": fits,
}
(ROOT / "outputs" / "SOURCE_FIXED_GEOGRAPHY_VALIDATION_AUDIT.json").write_text(
