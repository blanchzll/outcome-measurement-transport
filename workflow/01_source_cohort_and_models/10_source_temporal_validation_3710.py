...
SOURCE_ROOT = release_path("source")
SOURCE_WORKBOOK = SOURCE_ROOT / "inter3.xlsx"
SOURCE_COHORT = SOURCE_ROOT / "clean_clinical_data.csv"
RESULT_ROOT = release_path("analysis", "outputs/source3710_temporal")
...
if len(source) != 3710 or source["MajorID"].nunique() != 3710:
    raise ValueError("The locked primary cohort must contain 3710 unique MajorID values.")
if int(source[TARGET].sum()) != 152:
    raise ValueError("The locked primary cohort must contain 152 recorded events.")
# The locked CSV retains date strings for provenance, whereas Sheet1 is the
# authoritative, format-harmonised date source used for temporal analyses.
source = source.drop(
    columns=["AdmissionDate", "SurgeryDate", "DischargeDate"], errors="ignore"
)

dates_all = read_authoritative_sheet1_dates(SOURCE_WORKBOOK)
dates = dates_all.loc[dates_all["MajorID"].isin(source["MajorID"])].copy()
if len(dates) != 3710 or dates["MajorID"].nunique() != 3710:
    raise ValueError("All 3710 primary-cohort IDs must link once to authoritative Sheet1 dates.")
...
audit = {
    ...
    "source_workbook": "restricted_source/inter3.xlsx",
    "source_workbook_sha256": hashlib.sha256(SOURCE_WORKBOOK.read_bytes()).hexdigest(),
    "source_cohort": "restricted_source/clean_clinical_data.csv",
    "source_cohort_sha256": hashlib.sha256(SOURCE_COHORT.read_bytes()).hexdigest(),
    "parent_sheet1_n": int(len(dates_all)),
    "sheet": "Sheet1",
