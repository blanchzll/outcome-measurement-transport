...
SOURCE_PREDICTIONS = release_path("analysis", "secure_work/SOURCE_3710_LOCKED_LOCO_PREDICTIONS_SECURE.csv.gz")
RESULT_ROOT = release_path("analysis", "outputs/source3710_bayesian_calibration")
SEED = 20260827
N_DRAWS = 30000
...
# %%
d = pd.read_csv(SOURCE_PREDICTIONS, low_memory=False)
...
pd.DataFrame(parameter_rows).to_csv(tables / "Table_bayesian_hierarchical_calibration_parameters_3710.csv", index=False)
pd.DataFrame(centre_rows).to_csv(tables / "Table_bayesian_hierarchical_calibration_centres_3710.csv", index=False)
pd.DataFrame(sensitivity_rows).to_csv(tables / "Table_bayesian_hierarchical_calibration_prior_sensitivity_3710.csv", index=False)
...
(outputs / "BAYESIAN_HIERARCHICAL_CALIBRATION_3710_AUDIT.json").write_text(json.dumps(audit, indent=2) + "\n", encoding="utf-8")
